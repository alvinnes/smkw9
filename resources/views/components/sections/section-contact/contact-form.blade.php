<div class="w-full flex justify-between gap-18">
    <div class="w-7/12 h-136 rounded-md bg-white p-4" data-aos="fade-right">
        <div class="w-full h-90 bg-cover bg-no-repeat bg-position-[0_-10rem] rounded-md"
            style="background-image: url('{{ asset('img/school.jpg') }}')"></div>
        <div class="ml-1 bg-slate-100 h-42 p-10 rounded-md text-sm">
    ...
    <form action="/" method="post" class="w-5/12 h-full flex flex-col gap-5" data-aos="fade-left"
        data-aos-delay="200">
        @csrf
        <h2 class="text-4xl font-semibold leading-12 mb-6">Kontak untuk Kebaikan dan Kemajuan Bersama</h2>
        <x-input.input-item key="nama" placeholder="masukkan nama" icon="lucide-user-circle"
          ></x-input.input-item>
